...
# Define ROIs for jalan 1 and jalan 2 as polygons
roi_polygon_1 = np.array([(32, 136), (104, 127), (396, 429), (702, 576), (102, 576)], np.int32)
roi_polygon_2 = np.array([(109, 107), (149, 87), (303, 163), (701, 225), (700, 283), (232, 215)], np.int32)
...
# JSON file setup for each ROI
json_filename_1 = os.path.join('static/json', 'tugu_kujang_data_1.json')
json_filename_2 = os.path.join('static/json', 'tugu_kujang_data_2.json')
...
def generate_tugu_kujang_frames():
    global last_save_time
    video_path = "static/videos/tugu_kujang.mp4"  # or 0 for webcam
    ...
    if not os.path.exists(json_filename_1):
        with open(json_filename_1, 'w') as file:
            json.dump([], file)
    if not os.path.exists(json_filename_2):
        with open(json_filename_2, 'w') as file:
            json.dump([], file)
    ...
    while cap.isOpened():
        ...
        vehicle_counts_2 = {vehicle_type: 0 for vehicle_type in vehicle_sizes.keys()}
        total_occupied_area_1 = 0
        total_occupied_area_2 = 0

        # Draw ROIs for jalan 1 and jalan 2
        cv2.polylines(visualization_frame, [roi_polygon_1], True, (0, 255, 0), 2)
        cv2.polylines(visualization_frame, [roi_polygon_2], True, (0, 255, 0), 2)
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if class_name in vehicle_sizes:
                    ...
                    elif is_box_in_roi([x1, y1, x2, y2], roi_polygon_2):
                        vehicle_counts_2[class_name] += 1
                        total_occupied_area_2 += vehicle_sizes[class_name]['area']

        occupancy_percentage_1 = (total_occupied_area_1 / road_area_1_m2) * 100
        occupancy_percentage_2 = (total_occupied_area_2 / road_area_2_m2) * 100
        traffic_level_1, level_color_1 = get_traffic_level(occupancy_percentage_1)
        traffic_level_2, level_color_2 = get_traffic_level(occupancy_percentage_2)

        # Display information for jalan 1
        y_pos = 30
        text_gap = 20

        cv2.putText(visualization_frame, "Arah Tol Jagorawi", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.8, level_color_1, 2)
        y_pos += text_gap
        ...
        # Display information for jalan 2
        y_pos = 30
        text_gap = 20

        cv2.putText(visualization_frame, "Arah Kebun Raya Bogor", (400, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.8, level_color_2, 2)
        y_pos += text_gap

        for vehicle_type, count in vehicle_counts_2.items():
            color = colors.get(vehicle_type, (255, 255, 255))
            cv2.putText(visualization_frame, f"{vehicle_type}: {count}", (400, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
            y_pos += text_gap

        cv2.putText(visualization_frame, f"Occupancy: {occupancy_percentage_2:.1f}%", (400, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        y_pos += text_gap
        cv2.putText(visualization_frame, f"Status: {traffic_level_2}", (400, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, level_color_2, 2)

        current_time = time.time()
        if current_time - last_save_time >= 10:
            save_to_json(occupancy_percentage_1, json_filename_1)  # Pass filename for ROI 1
            save_to_json(occupancy_percentage_2, json_filename_2)  # Pass filename for ROI 2
