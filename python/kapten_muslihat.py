...
roi_polygon_2 = np.array([(534, 106), (567, 106), (1020, 571), (543, 553)], np.int32)
...
json_filename_2 = os.path.join('static/json', 'kapten_muslihat_2.json')
...
def generate_kapten_muslihat_frames():
    ...
    while cap.isOpened():
        ...
        occupancy_percentage_2 = (total_occupied_area_2 / road_area_m2) * 100
        ...
        cv2.putText(visualization_frame, "Arah Kebun Raya Bogor", (800, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.8, level_color_2, 2)
        y_pos += text_gap

        for vehicle_type, count in vehicle_counts_2.items():
            color = colors.get(vehicle_type, (255, 255, 255))
            cv2.putText(visualization_frame, f"{vehicle_type}: {count}", (800, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
            y_pos += text_gap

        cv2.putText(visualization_frame, f"Occupancy: {occupancy_percentage_2:.1f}%", (800, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        y_pos += text_gap
        cv2.putText(visualization_frame, f"Status: {traffic_level_2}", (800, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, level_color_2, 2)
